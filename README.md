...
- **🔎 Intelligent Code Search** - "Find where customfield_10531 is used in xena-jira-ai repo" (Server/DC only)
...
| **Bitbucket**     | Cloud              | ⚠️ Not Tested (Code search not available) |
| **Bitbucket**     | Server/Data Center | ✅ Supported (version 9.0+, includes code search) |
| **Xray for Jira** | Cloud              | ❌ Not Supported             |
| **Xray for Jira** | Server/Data Center | ✅ Supported (Jira 8.0+)     |

> [!NOTE]
> **Bitbucket Code Search**: Advanced code search features (`smart_search_code`, `deep_search_code`, `search_code_paginated`) are available exclusively on Bitbucket Server/Data Center. These features are not supported on Bitbucket Cloud due to API limitations. See [SEARCH_FEATURES.md](SEARCH_FEATURES.md) for detailed documentation on intelligent branch-aware search, pagination, and more.
...
**Code Search** (Server/Data Center only - see [SEARCH_FEATURES.md](SEARCH_FEATURES.md) for detailed documentation):
- `smart_search_code`: **DEFAULT** - Smart search that automatically chooses the best search strategy
- `deep_search_code`: Branch-aware search with automatic fallback across all branches
- `search_code_paginated`: Page-based navigation for search results (use only when explicitly requested)
- `search_code`: Low-level search with manual pagination control
...
\* Code search tools only available on Bitbucket Server/Data Center (not Cloud). See [SEARCH_FEATURES.md](SEARCH_FEATURES.md) for detailed documentation.
